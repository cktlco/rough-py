...
# Adapted. Copied directly from the svgelements python library for the
# purpose of removing the large number of conditional imports causing
# staggeringly slow runtime performance (90%+ of runtime is checking imports!)
...
from xml.etree.ElementTree import iterparse, Element, SubElement, ElementTree, tostring
        ...
        for event, elem in iterparse(source, events=("start", "end", "start-ns")):
            if event == "start":
                ...
                if SVG_ATTR_ID in attributes:  # If we have an ID, we save the node.
                    event_defs[attributes[SVG_ATTR_ID]] = (
